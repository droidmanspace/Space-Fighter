...
import net.simplifiedcoding.simplegame.BuildConfig;
import net.simplifiedcoding.simplegame.C0174R;
...
public class VectorDrawableCompat extends VectorDrawableCommon {
    ...
    private static class VGroup {
        ...
        public VGroup(VGroup copy, ArrayMap<String, Object> targetsMap) {
            ...
            for (int i = VectorDrawableCompat.LINEJOIN_MITER; i < children.size(); i += VectorDrawableCompat.LINEJOIN_ROUND) {
        ...
        private void updateStateFromTypedArray(TypedArray a, XmlPullParser parser) {
            ...
            this.mPivotX = a.getFloat(VectorDrawableCompat.LINEJOIN_ROUND, this.mPivotX);
            this.mPivotY = a.getFloat(VectorDrawableCompat.LINEJOIN_BEVEL, this.mPivotY);
            ...
            String groupName = a.getString(VectorDrawableCompat.LINEJOIN_MITER);
            if (groupName != null) {
    ...
    private static class VPath {
        ...
        public void printVPath(int level) {
            ...
            for (int i = VectorDrawableCompat.LINEJOIN_MITER; i < level; i += VectorDrawableCompat.LINEJOIN_ROUND) {
                indent = indent + "    ";
            }
        ...
        public String NodesToString(PathDataNode[] nodes) {
            String result = " ";
            for (int i = VectorDrawableCompat.LINEJOIN_MITER; i < nodes.length; i += VectorDrawableCompat.LINEJOIN_ROUND) {
                result = result + nodes[i].type + ":";
                float[] params = nodes[i].params;
                for (int j = VectorDrawableCompat.LINEJOIN_MITER; j < params.length; j += VectorDrawableCompat.LINEJOIN_ROUND) {
    ...
    private static class VPathRenderer {
        ...
        private void drawGroupTree(VGroup currentGroup, Matrix currentMatrix, Canvas canvas, int w, int h, ColorFilter filter) {
            ...
            for (int i = VectorDrawableCompat.LINEJOIN_MITER; i < currentGroup.mChildren.size(); i += VectorDrawableCompat.LINEJOIN_ROUND) {
        ...
        private float getMatrixScale(Matrix groupStackedMatrix) {
            ...
            float scaleX = (float) Math.hypot((double) unitVectors[VectorDrawableCompat.LINEJOIN_MITER], (double) unitVectors[VectorDrawableCompat.LINEJOIN_ROUND]);
            float scaleY = (float) Math.hypot((double) unitVectors[VectorDrawableCompat.LINEJOIN_BEVEL], (double) unitVectors[3]);
            float crossProduct = cross(unitVectors[VectorDrawableCompat.LINEJOIN_MITER], unitVectors[VectorDrawableCompat.LINEJOIN_ROUND], unitVectors[VectorDrawableCompat.LINEJOIN_BEVEL], unitVectors[3]);
    ...
    private static class VClipPath extends VPath {
        ...
        private void updateStateFromTypedArray(TypedArray a) {
            String pathName = a.getString(VectorDrawableCompat.LINEJOIN_MITER);
            if (pathName != null) {
                this.mPathName = pathName;
            }
            String pathData = a.getString(VectorDrawableCompat.LINEJOIN_ROUND);
            if (pathData != null) {
    ...
    private static class VFullPath extends VPath {
        ...
        private void updateStateFromTypedArray(TypedArray a, XmlPullParser parser) {
            ...
            if (TypedArrayUtils.hasAttribute(parser, "pathData")) {
                ...
                String pathData = a.getString(VectorDrawableCompat.LINEJOIN_BEVEL);
                if (pathData != null) {
                    this.mNodes = PathParser.createNodesFromPathData(pathData);
                }
                this.mFillColor = TypedArrayUtils.getNamedColor(a, parser, "fillColor", VectorDrawableCompat.LINEJOIN_ROUND, this.mFillColor);
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.mTmpBounds.width() > 0 && this.mTmpBounds.height() > 0) {
            ...
            float canvasScaleX = Math.abs(this.mTmpFloats[LINEJOIN_MITER]);
            float canvasScaleY = Math.abs(this.mTmpFloats[4]);
            float canvasSkewX = Math.abs(this.mTmpFloats[LINEJOIN_ROUND]);
    ...
    private static Mode parseTintModeCompat(int value, Mode defaultMode) {
        switch (value) {
            ...
            case WearableExtender.SIZE_FULL_SCREEN /*5*/:
                return Mode.SRC_IN;
            case C0174R.styleable.Toolbar_popupTheme /*9*/:
                return Mode.SRC_ATOP;
            case C0174R.styleable.Toolbar_titleMarginEnd /*14*/:
                return Mode.MULTIPLY;
            case C0174R.styleable.Toolbar_titleMarginTop /*15*/:
                return Mode.SCREEN;
    ...
    private void printGroupTree(VGroup currentGroup, int level) {
        ...
        for (i = LINEJOIN_MITER; i < currentGroup.mChildren.size(); i += LINEJOIN_ROUND) {
            ...
            if (child instanceof VGroup) {
                ...
                ((VPath) child).printVPath(level + LINEJOIN_ROUND);
            }
        }
    }
